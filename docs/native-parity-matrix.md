...
| **11** | **三指拖移与分阶段换把<br>(3F Drag & Regrip)** | 经典三指拖移：手指位移即按住左键拖拽；本项目支持显式 `3F → 0F → 4F → 0F → 3F` staged 会话，跨 Space 后继续抓取同一窗口 | `0.35mm` 触发门限；默认 `persistent_drag_lock=true`；完整抬手进入 `DragLocked`，4F 阶段保持 `drag_button_held=true`，再落 3F 恢复移动；1F/2F 短触摸解锁 | • 原生：普通三指拖移抬手即停止，辅助功能 Drag Lock 是另一语义<br>• 本项目：持久锁是 companion 扩展；`persistent_drag_lock=false` 时才使用有限 `release_delay_ms` | **自动化覆盖；系统私有事件与真机手感待验** |
