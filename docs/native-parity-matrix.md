...
| **09** | **双指缩放与 360° 旋转<br>(Pinch & 360° Rotate)** | 相对增量流；缩放与旋转共享双指手势族，通过主导权滞回（Hysteresis）防止抖动互串；遵循 AppKit `NSEvent.rotation` 逆时针为正标准 | 恢复 `PINCH_ROTATE_HYSTERESIS=1.5`；**旋转角增量严格取反 `-angle_d.to_degrees()` 以契合 macOS 逆时针为正规范**；缩放主导时严格静音旋转流 | • 原生：顺时针为负，逆时针为正；1.5 倍滞回<br>• 本项目：完全对齐 AppKit 符号规范，$4.0^\circ$ 死区 | **已完全对齐**<br>(顺时针旋转同向旋转，捏合缩放不再晃动) |
