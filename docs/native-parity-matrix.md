...
| **11** | **三指拖移与分阶段换把<br>(3F Drag & Regrip)** | 经典三指拖移：手指位移即按住左键拖拽；本项目支持显式 `3F → 0F → 4F → 0F → 3F` staged 会话，跨 Space 后继续抓取同一窗口 | `0.35mm` 触发门限；默认 `persistent_drag_lock=true`；完整抬手进入 `DragLocked`，4F 阶段保持 `drag_button_held=true`，再落 3F 恢复移动；静止 1F/2F 解锁，真实移动立即回普通输入 | • 原生：普通三指拖移抬手即停止，辅助功能 Drag Lock 是另一语义<br>• 本项目：持久锁是 companion 扩展；`persistent_drag_lock=false` 时才使用有限 `release_delay_ms` | **自动化覆盖；系统私有事件与真机手感待验** |
| **12** | **四指桌面平移切换<br>(4F Spaces Swipe)** | 四指横扫通过 DockControl / SkyLight 驱动多桌面平滑过渡动画 | **手指数跳变重锚**（$4 \to 3$ 或 $3 \to 4$ 时几何重置，累加 `cumulative_dx`，避免丢指跳变）；macOS 26 保留 DockSwipe，macOS 27+ 优先走 `SLEventSetIOHIDEvent` 连续 HID payload，运行时不可用再降级 SymbolicHotKey | • 原生行程：$50\text{mm}$ 对应 1.0 满行程进度<br>• 本项目：`SWIPE_PROGRESS_REF_MM=50.0mm`；27+ fallback 为离散切换 | **联合拖拽状态机已覆盖；私有/系统路径待 macOS 版本实测** |
...
| **16** | **双指右边缘滑入通知中心<br>(2F Right Edge Swipe)** | 双指从触控板右边缘向左滑入展开/收起通知中心 | 边缘区域起始 $x \ge 0.85 \times \text{SurfaceWidth}$，向左滑动 $\Delta x \le -3.8\text{mm}$ 实时触发通知中心唤出；派发 SkyLight HotKey 163 与 ControlCenter 时钟锚点 | • 原生右缘判定：$x \ge \text{EdgeZone}$，$\Delta x \le -3.5\text{mm}$<br>• 本项目：两根手指都从配置宽度最后 15% 开始，默认 `surface_width_mm=65`，$\Delta x \le -3.8\text{mm}$ 实时触发 | **触控区模拟 + 私有热键；待真机** |
...
| 拖移锁定与换把悬停 (Drag Lock) | `Dragging` / `DragLock` | `[gestures.three_finger_drag].persistent_drag_lock` + `release_delay_ms` | `persistent_drag_lock=true` 为 staged 3F/4F/3F 会话；关闭后才使用有限 `release_delay_ms`，两者不冒充同一个 Apple 私有开关 |
...
* **解法**：
  * macOS 27+ 采用 `SkyLight.framework` 的 `SLEventSetIOHIDEvent` 动态构建 `HIDEvent`（type 23 + type 9 速度子事件），不可用时降级到 SymbolicHotKey；macOS 26 及更早版本保持经典完整 12 字段 CGEvent 形状；
