...
- [x] X8. Android 连接弹窗统一为“扫码优先、Bonjour 发现、IP 备用”的可滚动流程；相机权限或扫码页不可用时明确回退到 IP 连接，保留 Token 校验和 UDP-only 探测。
...
- [x] X10. 移除 Google Play Code Scanner 的动态 `barcode_ui` 依赖，改为 APK 内置 ML Kit 模型和本地相机页；相机权限、取消、无摄像头和无效二维码均可回到 IP 连接。

阶段 X：**代码、Android 单测/Debug 构建和 ADB 真机连接页验收已完成；扫码模型随 APK 内置，不依赖 Google Play 动态模块；目标 Mac 的 SwiftUI、Bonjour 网络隔离和 DMG 最终验收仍待 Mac 执行。**
...
- [x] Z4f. 应用退出时同步停止 `companion-net`，等待并在必要时强制结束 helper，避免
  立即重开应用与旧实例争抢 `instance.lock`；deinit 也保留同一清理兜底。
- [ ] Z4g. 将剩余双语文案迁移到独立 `.strings` 资源并加入本地化快照测试。

阶段 Z：**Z1-Z4f 代码已完成并通过静态检查；PermissionFlow、网络切换、登录项和
helper 恢复仍需在 macOS 真机/签名 DMG 上验收，Z4g 本地化资源化待后续迭代。**
