...
| Four-finger full-screen app/Spaces | DockSwipe 私有 payload；macOS 26 用 SymbolicHotKey，27+ 尝试 SkyLight HIDEvent | C/T | macOS 版本分支和私有 ownership/phase/velocity 均待真机 |
...
| 3F drag + 4F Spaces | 进入 `FourFingerLive` 不松左键；macOS 26 用 symbolic hotkey，27+ 尝试 HIDEvent | Apple 没公开并行合同；26 fallback 是离散跳转，不能持续 rubber-band | C/T |
