...
|---|---|---|
| 三指拖移 (3-finger drag) | Resting 3 fingers then moving drags windows / selects text. The companion can keep the left button held across a complete lift and a four-finger Space stage, then resume on a fresh three-finger grip | **Implemented, on by default** (`GestureKind::ThreeFingerDrag` + `DragLocked`, `[gestures.three_finger_drag].persistent_drag_lock = true`). The supported staged contract is `3F → 0F → 4F → 0F → 3F`; a fourth finger added before the complete lift releases the live drag. A short 1F/2F touch unlocks. Set `persistent_drag_lock = false` to use finite `release_delay_ms` re-grip behavior, or `release_delay_ms = 0` for immediate release. This is synthesized mouse behavior, not Apple's raw drag-lock stream |
