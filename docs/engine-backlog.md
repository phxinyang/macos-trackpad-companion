...
| 三指拖拽 + 四指切 Space | Hold a window with three fingers, add a fourth finger, switch Space, then release | **Implemented state transition; carried drags use SymbolicHotKey on macOS 26+ (standalone macOS 26 swipes keep DockSwipe; macOS 27+ prefers HIDEvent and falls back to SymbolicHotKey)** — left button remains held through the switch, horizontal threshold 10mm, vertical threshold 7mm, 350ms cooldown. Requires macOS application/WindowServer verification |
