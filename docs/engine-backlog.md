...
|---|---|---|
| 三指拖移 (3-finger drag) | Resting 3 fingers then moving drags windows / selects text. The companion can keep the left button held across a complete lift and a four-finger Space stage, then resume on a fresh three-finger grip | **Implemented, on by default** (`GestureKind::ThreeFingerDrag` + `DragLocked`, `[gestures.three_finger_drag].persistent_drag_lock = true`). The supported staged contract is `3F → 0F → 4F → 0F → 3F`; a fourth finger added before the complete lift releases the live drag. A stationary 1F/2F touch unlocks, while real 1F/2F motion exits to ordinary input. Set `persistent_drag_lock = false` to use finite `release_delay_ms` re-grip behavior, or `release_delay_ms = 0` for immediate release. This is synthesized mouse behavior, not Apple's raw drag-lock stream |
...
| 三指拖拽 + 四指切 Space | Hold a window with three fingers, lift, use four fingers to switch Space, lift, then re-grip with three fingers before release | **Implemented staged state transition; `DragLocked` carries the left button through the 4F Space stage and resumes on 3F. Direct 3F→4F addition releases the live drag. macOS 26+ carried swipes use SymbolicHotKey while standalone swipes retain their version-specific DockSwipe/HIDEvent path.** Requires macOS application/WindowServer verification |
