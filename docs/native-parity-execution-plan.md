...
- [x] U1. Web/Android 将壁纸作为独立画布层，选中图片时不再叠加主题渐变，避免重复染色。
- [x] U2. 增加背景可见度、饱和度、亮度和触控面透明度；非液态主题用稳定的透底 surface，Liquid Glass 将同一透明度作为材质 alpha。
- [x] U3. Android 壁纸使用 `ColorMatrix` 和低强度可读性 scrim，API 31+ GPU 玻璃与旧版回退共享该场景；Web 背景伪元素保持输入层透明。
- [x] U4. 浏览器截图、Web 脚本语法、Kotlin 编译、ARM AAPT2 重打包和 ADB 真机视觉回归均已完成；新版 APK 已安装到 `192.168.3.137:44899`，SHA-256 为 `698b56090262b3088c0b3e006bd78496b29d1751020e4706b238a7e709861a7d`。

阶段 U：**代码、自动化检查、APK 构建、ADB 安装和真机验收均已完成。**

### 阶段 V：全屏材质连续性与动效收敛（2026-08-30）

- [x] V1. Android 全屏切换改为 chrome-only 状态：保持切换前的可见外边距
  （连接态为 18dp）、8dp 内填充、30dp 圆角和当前表面透明度，只对触控面
  做 0.985/0.995 的轻微 transform settle，消除亮壁纸上的 alpha 闪变。
- [x] V2. Android 顶栏与退出按钮使用短时可中断的 ease-out 曲线，退出按钮
  从右上角同步缩放淡入/淡出；全屏期间持续保留深按条和触控面命中区域。
- [x] V3. Web 全屏移除旧的 `inset: 0`、零圆角和无阴影规则，桌面/窄屏均
  保留居中液态玻璃；浮动设置按钮改为可见性状态动画，避免 `display` 跳变。
- [x] V4. 完成 `testDebugUnitTest assembleDebug`、Web 内联脚本语法检查，
  在 `192.168.3.137:44899` 重装并抓取普通、60ms、240ms、稳定全屏截图；
  触控面区域前后平均像素差约 0.62%，差异来自顶栏与退出按钮，未出现黑边或
  整屏材质重置。APK SHA-256：`93c35a4b9f2a84d0301ff40ce6be3e81720dd13511241481c4149696559d22c1`。

阶段 V：**代码、自动化检查、APK 构建、ADB 安装和真机视觉回归均已完成；目标 Mac 的最终应用消费矩阵仍按阶段 T6/N7 执行。**

### 阶段 W：全屏退出与 Web 兼容回退（2026-08-30）

- [x] W1. Android 覆盖系统返回键：全屏状态下优先退出 chrome-only 模式，保留
  配对会话和触控面；非全屏仍交给 Activity 默认返回行为。
- [x] W2. Web 检测 Fullscreen API 能力；iOS Safari/嵌入式 WebView 不支持时，
  使用同一套 chrome-only CSS 状态，浮动设置按钮仍可退出，不再让全屏操作静默失败。
- [x] W3. 完成 Web 内联脚本检查、Android 单测/Debug 构建和 ADB 真机返回键回归；
  真机确认返回后顶栏、连接状态和触控面保持可用。
- [x] W4. `companion-net` 在现有 TCP 端口提供可选 token 保护的 `GET /health`；
  Android 连接前执行探针，旧版无 token 服务的 `404` 保持兼容，错误 token 不再显示
  为已连接。最新 APK SHA-256：`a51bfe83edf634e25cce7d7547a308390ee90320ff76fb5334d23d9c7a020b90`。

阶段 W：**代码、自动化检查、APK 构建、ADB 安装和返回键真机验收均已完成。**
