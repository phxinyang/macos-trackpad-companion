...

{
    ...
    echo "TrackpadTwoFingerDoubleTapGesture: $(defaults read -g com.apple.trackpad.twoFingerDoubleTapGesture 2>/dev/null || echo '未设置')"
    echo "TrackpadThreeFingerTapGesture: $(defaults read -g com.apple.trackpad.threeFingerTapGesture 2>/dev/null || echo '未设置')"
    echo "TrackpadThreeFingerDrag: $(defaults read -g com.apple.trackpad.threeFingerDrag 2>/dev/null || echo '未设置')"
    echo "TrackpadPinch: $(defaults read -g com.apple.trackpad.pinch 2>/dev/null || echo '未设置')"
    echo "TrackpadRotate: $(defaults read -g com.apple.trackpad.rotate 2>/dev/null || echo '未设置')"
    echo "TrackpadScaling: $(defaults read -g com.apple.trackpad.scaling 2>/dev/null || echo '未设置')"
    echo "NaturalScroll: $(defaults read -g com.apple.swipescrolldirection 2>/dev/null || echo '未设置')"
    echo ""
