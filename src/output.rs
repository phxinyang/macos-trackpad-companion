...
// macOS 26 (Tahoe) rejects synthetic DockSwipe events generated by
// third-party processes. Mac Mouse Fix's PR #1875 uses the WindowServer's
// SymbolicHotKey registry there; macOS 27 can use the repaired HIDEvent path
// and shares this state machine only as a runtime fallback. These distances
...
impl Emitter {
    ...
    fn swipe_synthetic(
        ...
        // macOS 26 rejects third-party DockSwipe payloads, so route that
        // release through the system's symbolic hot-key registry. macOS 27
        // restored the continuous path when the HIDEvent payload is attached
        // with SkyLight's SLEventSetIOHIDEvent; keep that path available for
        // the native rubber-band feel and only fall back to a hot-key if the
        // private setter is unavailable at runtime. The left-button state is
        // untouched in either case, so a three-finger window drag remains
        // attached while Spaces changes.
        if (macos_26_or_later() && !macos_27_or_later()) || self.symbolic_swipe_active.get() {
    ...
    /// hot-key presses on macOS 26 or when the macOS 27+ HID path is
...
fn macos_26_or_later() -> bool {
    use std::sync::OnceLock;
    static RESULT: OnceLock<bool> = OnceLock::new();
    // Unknown versions use the safer SymbolicHotKey route. A rejected
    // DockSwipe can do nothing, while a symbolic Space hot-key remains a
    // normal WindowServer action and is easy to observe/recover from.
    *RESULT.get_or_init(|| macos_major_version().map_or(true, |major| major >= 26))
}
